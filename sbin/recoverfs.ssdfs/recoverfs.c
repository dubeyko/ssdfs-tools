				  ...
				  u64 timestamp)
{
	...
	state->checkpoint_fd = openat(state->output_fd, state->name_buf,
					O_DIRECTORY, 0777);
	if (state->checkpoint_fd < 1) {
				     ...
				     struct ssdfs_block_descriptor *blk_desc)
{
	 ...
	 */
	fd = openat(state->checkpoint_fd, state->name_buf,
		    O_RDWR | O_LARGEFILE,
	...
	if (fd < 0) {
		...
		close(fd);
		SSDFS_ERR("file exists already %s: %s\n",
			  state->name_buf, strerror(errno));
...
int main(int argc, char *argv[])
{
	...
	if (env.output_fd == -1) {
		err = mkdir(env.output_folder, 0777);
		if (err < 0) {
			close(env.base.fd);
	...
	for (i = 0; i < env.threads; i++) {
		...
		err = ssdfs_create_raw_dump_environment(&jobs[i].base,
							&jobs[i].raw_dump);
		if (err) {
...
free_threads_pool:
	if (jobs) {
		for (i = 0; i < env.threads; i++) {
			ssdfs_destroy_raw_dump_environment(&jobs[i].raw_dump);
		}
		free(jobs);
	}
