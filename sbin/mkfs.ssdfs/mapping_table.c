				 ...
				 u32 blks)
{
	...
	if (pages_per_peb % layout->maptbl.log_pages) {
		SSDFS_WARN("pages_per_peb %u, log_pages %u\n",
			   pages_per_peb,
			   layout->maptbl.log_pages);
	}

	if (layout->maptbl.log_pages < blks) {
		SSDFS_WARN("log_pages is corrected from %u to %u\n",
			   layout->maptbl.log_pages, blks);
		layout->maptbl.log_pages = blks;
	} else if (layout->maptbl.log_pages % blks) {
		SSDFS_WARN("log_pages %u, blks %u\n",
			   layout->maptbl.log_pages,
			   blks);
	}

	layout->sb.vh.maptbl_log_pages = cpu_to_le16(blks);
}
